...
\documentclass[leqno,onefignum,onetabnum]{siamltexmm}
\usepackage{graphicx}
\usepackage{amsmath}
%\usepackage{fullpage}
\usepackage{amssymb}
...
\usepackage[colorlinks,pdfmark,dvips]{hyperref}
...
\end{equation*}
 the matrices $\Wbar_k = {\sf diag}(\bfw_0,\hdots,\bfw_k)$,
and $ \Qbar^{-1}_k = {\sf blkdiag}(\bfQ^{-1}_1,\hdots,\bfQ^{-1}_k)$, and the vectors $\widehat{\mbar}_k = (\bfmhat_0^{\top}, \hdots, \bfmhat_k^{\top})^{\top}$, and $\mbar_k = (\bfm_0^{\top}, \hdots, \bfm_k^{\top})^{\top}$,   
