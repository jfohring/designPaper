...
\documentclass[final,leqno,onetabnum]{siamltexmm}
...
First a linear inversion was performed where the operator $\bfL$ was chosen to be $\bfI$,  $\bfL = \bfI$, to promote smallness in the recovered models. Alternately, we could have assigned $\bfL = \GRADh$ to promote smooth edges in the models. However, since the model has sharp edges, it does not make sense to design for smoothness. 
...
The covariance matrices $\bfQ_k$ were assigned a scalar value for the variance for each time step to represent iid noise in the dynamics, such that $\bfQ_k = \sigma_k \bfI$. We assigned a constant $\sigma_k = \pm 0.2$, corresponding to $20\%$ noise, for all experiments.
