...
The noisy design optimization problem is constructed in exactly the same fashion as the  noiseless case, by minimizing the {\sf amse} for $\bfw_k$ with a sparsity penalty on $\bfw_k$. 
\begin{eqnarray*}
...
Two different inversions were carried out to recover models $\bfm_k$. First a linear inversion was performed with the gradient operator, and second a non-linear inversion was performed with smoothed total variation as the  regularization ~\cite{Ascher2006} because it is possible that the models can have discontinuities or sharp boundaries. This is particularly valid for our experiment since we have chosen advection without any diffusion term to transport a concentration of tracer. In this ideal case there should be no diffusion of the tracer. 
